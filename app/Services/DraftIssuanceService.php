...
class DraftIssuanceService
{
     ...
     */
    public function scrapedraftIssuance(string $url, $search = null)
    {
        $client = new Client([
        ...
        try {
            ...
            while ($url) {
                ...
                // Scrape all rows
                $drafts = $crawler->filter('table.view_details tr')->each(function (Crawler $node) use ($client, $search) {
                    try {
                        // Extract main data
                ...
                $drafts = array_filter($drafts);
                foreach ($drafts as $draft) {
                    if (!array_key_exists($draft['reference'], $uniqueDrafts)) {
                        $uniqueDrafts[$draft['reference']] = $draft;

                        // **Store the download link in the database**
                        DraftIssuance::updateOrCreate(
                            ['reference' => $draft['reference']], // Unique constraint
                            [
                                'title' => $draft['title'],
                                'link' => $draft['link'],
                                'date' => $draft['date'],
                                'download_link' => $draft['downloadLink'], // <-- Store the download link
                            ]
                        );
                    }
                }
            ...
            return [
                'acts' => array_values($uniqueDrafts),
            ];
        } catch (\Exception $e) {
